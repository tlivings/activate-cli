...
must_haves:
  ...
  artifacts:
    ...
    - path: "README.md"
      ...
      contains: ["Installation", "Usage", "Configuration"]
  key_links:
    - from: "src/tui/app.rs"
      to: "src/commands/deactivate.rs"
      via: "deactivate_request triggers execute_deactivate"
...
<objective>
Add TUI enhancements (deactivate/archive actions, help overlay) and create project README.

Purpose: Complete TUI requirements and provide documentation for users to install and use activate.
...
4. Add helper methods for updating project state after action:
```rust
...
if let Some(name) = app.deactivate_request.take() {
    ...
    if let Some(project) = app.projects.iter().find(|p| p.name == name) {
        // Check for uncommitted changes (show in status area later if needed)
        if let Ok(status) = GitStatus::check(&project.path) {
            if status.has_warnings() {
                // Could show warning in TUI status, for now just log
                // The warning will appear briefly before TUI refreshes
            }
        }
    }
...
if let Some(name) = app.archive_request.take() {
    ...
    if let Some(project) = app.projects.iter().find(|p| p.name == name) {
        if let Ok(status) = GitStatus::check(&project.path) {
            if status.has_warnings() {
                // Could show warning in TUI status
            }
        }
    }
...
# List all projects
activate list
activate list --active   # Filter by state
...
Keep it simple and feature-focused per CONTEXT.md specifics.
  </action>
  <verify>
File exists: `ls README.md`
Content is valid markdown (visual check)
  </verify>
  <done>
README.md exists with installation, usage, configuration, and keybinding documentation
  </done>
</task>
...
6. README.md exists with installation and usage sections
</verification>

<success_criteria>
- TUI supports d/a/? keys for deactivate, archive, help
- Help overlay is centered and shows all keybindings
- README documents installation, shell setup, usage, and TUI keys
