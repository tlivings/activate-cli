...
mod tests {
    ...
    #[test]
    fn test_deactivate_active_project() {
        ...
        update_project_state(&db.conn, "my-project", ProjectState::Active).unwrap();

        // Now deactivate
        execute_deactivate(&db, "my-project").unwrap();
    ...
    #[test]
    fn test_deactivate_fuzzy_match() {
        let db = setup_test_db().unwrap();
        let temp_dir = TempDir::new().unwrap();
        ...
        update_project_state(&db.conn, "my-awesome-project", ProjectState::Active).unwrap();

        // Deactivate with fuzzy name
        execute_deactivate(&db, "awesome").unwrap();
    ...
    fn test_deactivate_not_found() {
        let db = setup_test_db().unwrap();
        let result = execute_deactivate(&db, "nonexistent");
